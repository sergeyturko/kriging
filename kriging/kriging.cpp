...
kriging::kriging(float mT, int rmf, int rf) : majorityTresh(mT), radiusKernelMF(rmf), radiusF(rf)
{}

bool kriging::read(char* fname)
{
	CurrentImages = cv::imread(fname);
	if (CurrentImages.rows == 0)
		return false;

	if (CurrentImages.channels() > 1)
	{
		cv::cvtColor(CurrentImages, CurrentImages, CV_RGB2GRAY);
		return true;
	}
	return true;
}
